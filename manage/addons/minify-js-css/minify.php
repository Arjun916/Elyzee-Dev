...
class MinifyJsCss{
    static function minify_js_css( $params, $node ){
        ...
        if($filetype != 'css' && $filetype != 'js' && $filetype != 'schema'){die("ERROR: Tag \"".$node->name."\" - Must specify either 'css', 'js' or 'schema'.");}
        $filepath = trim(str_replace( K_SITE_URL, "", $params[1]['rhs'] ));
		$fileasync = strtolower(trim($params[2]['rhs']));
		...
		if($fileasync && $fileasync == '1'){
		$css_tag .= '<noscript><link rel="stylesheet" href="'.$output_link;
		if(defined('MINIFY_TIMESTAMP_QUERYSTRING')){$css_tag .= '?'.$last_mod;}
		$css_tag .= '></noscript>';
		}
        ...
        if ($filetype == 'js' || $filetype == 'schema'){ 
        ...
        if(!$output_file){ 
			...
			if ($filetype == 'schema'){ return '<script type="application/ld+json"' . $tag_attributes . '>' . $output . '</script>'; }
        }
        
		error_log("MINIFY output_file: ".$output_file);
		error_log("MINIFY output_dir writable? ".(is_writable(dirname($output_file)) ? 'YES' : 'NO'));
