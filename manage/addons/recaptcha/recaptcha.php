...
class KReCaptchaForm extends KUserDefinedFormField{
    ...
    function _render( $input_name, $input_id, $extra='', $dynamic_insertion=0 ){
        ...
        if( !$count ){
            ...
            ?>
            <script>
                window.k_recaptchas = [ {name:"<?php echo $input_id. '_k_'.$count; ?>",theme:"<?php echo $this->theme; ?>",size:"<?php echo $this->size; ?>",hl:"<?php echo $this->hl; ?>"} ];
            ...
            if( is_null($this->cached) ){
                ...
                if( $ok ){
                    $html = '<script>window.k_recaptchas.push( {name:"'.$input_id.'_k_'.$count.'",theme:"'.$this->theme.'",size:"'.$this->size.'",hl:"'.$this->hl.'"} );</script>';
